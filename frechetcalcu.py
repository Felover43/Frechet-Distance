...
def _c_multi(ca,i,j,k,P,Q,R):
    ...
    if (int(ca[i,j,k]) > -1):
        ...
        # first col
        ca[i,j,k] = max(_c_multi(ca,i-1,0,0,P,Q,R)[0],euc_dist(P[i],Q[0]),euc_dist(P[i],R[0]),euc_dist(Q[0],R[0]))

    elif i == 0 and j > 0 and k == 0:ca[i,j,k] = max(_c_multi(ca,0,j-1,0,P,Q,R)[0],euc_dist(P[0],Q[j]),euc_dist(P[0],R[0]),euc_dist(Q[j],R[0]))

    elif i == 0 and j == 0 and k > 0:ca[i,j,k] = max(_c_multi(ca,0,0,k-1,P,Q,R)[0],euc_dist(P[0],Q[0]),euc_dist(P[0],R[k]),euc_dist(Q[0],R[k]))

    elif i > 0 and j > 0 and k == 0:ca[i,j,k] = max(min(_c_multi(ca,i-1,j,0,P,Q,R)[0],_c_multi(ca,i-1,j-1,0,P,Q,R)[0],_c_multi(ca,i,j-1,0,P,Q,R)[0]),euc_dist(P[i],R[0]),euc_dist(Q[j],R[0]),euc_dist(P[i],Q[j]))

    elif i > 0 and j == 0 and k > 0:ca[i,j,k] = max(min(_c_multi(ca,i-1,0,k,P,Q,R)[0],_c_multi(ca,i-1,0,k-1,P,Q,R)[0],_c_multi(ca,i,0,k-1,P,Q,R)[0]),euc_dist(P[i],R[k]),euc_dist(Q[0],R[k]),euc_dist(P[i],Q[0]))

    elif i == 0 and j > 0 and k > 0:ca[i,j,k] = max(min(_c_multi(ca,0,j-1,k,P,Q,R)[0],_c_multi(ca,0,j-1,k-1,P,Q,R)[0],_c_multi(ca,0,j,k-1,P,Q,R)[0]),euc_dist(P[0],R[k]),euc_dist(Q[j],R[k]),euc_dist(P[0],Q[j]))

    elif i > 0 and j > 0 and k > 0:ca[i,j,k] = max(min(_c_multi(ca,i-1,j-1,k,P,Q,R)[0],_c_multi(ca,i,j-1,k,P,Q,R)[0],_c_multi(ca,i-1,j,k,P,Q,R)[0],_c_multi(ca,i-1,j-1,k-
